...
$idPairs = [
    'reeks' => ['Reeks_Index' => $_POST['index'], 'Reeks_Naam' => $_POST['naam']],
    'items' => ['Item_Reeks' => $_POST['index']],
    'collectie' => ['Reeks_Index' => $_POST['index']
]];
...
if(is_string($remReeks)) {
    App::resolve('session')->flash(['feedback' => $remReeks]);
    return App::redirect('beheer', TRUE);
}
...
if(is_string($remCollectie)) {
    App::resolve('session')->flash(['feedback' => $remCollectie]);
    return App::redirect('beheer', TRUE);
}

/* Set the feedback message for the user, in the _flash memory. */
App::resolve('session')->flash('feedback', ['success' => "De reeks: {$idPairs['reeks']['Reeks_Naam']} en al de items en collecties ervan zijn verwijderd !"]);
/* Unset the items and update the reeks page-data to reflect the change. */
