...
if(isset($_SESSION['_flash']['tags']['reeks-index'])) { $ids = ['Item_Reeks' => $_SESSION['_flash']['tags']['reeks-index']]; }

if(isset($_SESSION['page-data']['huidige-reeks']) && !isset($ids['Item_Reeks'])) {
    $ids = ['Item_Reeks' => App::resolve('reeks')->getKey(['Reeks_Naam' => $_SESSION['page-data']['huidige-reeks']], 'Reeks_Index')];
}

/* Always refresh the loaded items, if the ids are set. */
if(isset($ids)) {
    if(isset($_SESSION['page-data']['items'])) { unset($_SESSION['page-data']['items']); }
    App::resolve('session')->setVariable('page-data', ['items' => App::resolve('items')->getAllFor($ids)]);
}
