...
<script>
    ...
    if(localStorage.checkState) {
        ...
        const checkbox = document.getElementById(stateKey); checkbox.checked = stateVal;
        /* The span text loop, changing depending on what checkbox was checked. */
        if(checkbox === 'item-zoek-naam-inp' && stateVal) { span.innerHTML = 'Zoek op item naam'; } else if(checkbox === 'item-zoek-nr-inp' && stateVal) { span.innerHTML = 'Zoek op item nummer'; } else if(checkbox === 'item-zoek-isbn-inp' && stateVal) { span.innerHTML = 'Zoek op item isbn'; }
        /* What todo if a checkbox was unselected. */
        if(!stateVal) { span.innerHTML = 'Selecteer een zoek optie ..'; itemZoekInp.disabled = true; }
    ...
    function checkBoxSearch(event) {
        ...
        const checkArr = new Array(event.target.id, event.target.checked); localStorage.setItem('checkState', JSON.stringify(checkArr));
        
        /* If the checkbox was checked, create a new input event, */
        if(event.target.checked === true) {
            let inputEvent = new Event ('input', {'bubbles': true,'cancelable': false});
            /* If the input was disabled, i enable it for the user. */
            if(itemZoekInp.disabled) { itemZoekInp.disabled = false; }
            ...
            if(itemZoekInp.value) { return itemZoekInp.dispatchEvent(inputEvent); }
    ...
    function itemZoek(e) {
        ...
        if(chb1.checked === true) {
            tafelRows.forEach((item, index) => {
                /* Change the child used, based on the url pathname. */
                if(document.location.pathname == '/gebruik'){ itemNaam = item.children[1].innerHTML; } else { itemNaam = item.children[2].innerHTML; }
                /* Check the html item name, against the user input item naam, and dont display the mismatches. */
                if(itemNaam.toUpperCase().indexOf(filter) > -1) { return tafelRows[index].style.display = ''; } else { return tafelRows[index].style.display = 'none'; }
            });
        /* We are searching on a album nr basis */
        } else if(chb2.checked === true) {
            tafelRows.forEach((item, index) => {
                /* Change the child used, based on the url pathname. */
                if(document.location.pathname == '/gebruik'){ itemNr = item.children[2].innerHTML; } else { itemNr = item.children[3].innerHTML; }
                /* Check the html item nr, against the user input item nr, and dont display the mismatches. */
                if(itemNr.toUpperCase().indexOf(filter) > -1) { return tafelRows[index].style.display = ''; } else { return tafelRows[index].style.display = 'none'; }
            });
        /* We are searching on a album isbn basis */
        } else if(chb3.checked === true) {
            tafelRows.forEach((item, index) => {
                /* Change the child used, based on the url pathname. */
                if(document.location.pathname == '/gebruik'){ itemIsbn = item.children[6].innerHTML; } else { itemIsbn = item.children[7].innerHTML; }
                /* Check the html item isbn against the user input item isbn, and dont display the mismatches. */
                if(itemIsbn.toUpperCase().indexOf(filter) > -1) { return tafelRows[index].style.display = ''; } else { return tafelRows[index].style.display = 'none'; }
            });
        }
    }
</script>