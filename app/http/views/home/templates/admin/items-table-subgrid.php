...
<script>
    ...
    const bewButt = document.getElementsByClassName('item-bew-butt'); const bewButtArr = Array.from(bewButt); for(key in bewButtArr) { bewButtArr[key].addEventListener('click', saveScroll); }
    /* Add listen events to all delete buttons. */
    const verwButt = document.getElementsByClassName('item-verwijderen-butt'); const verwButtArr = Array.from(verwButt); for(key in verwButtArr) { verwButtArr[key].addEventListener('click', saveScroll); }
    ...
    function itemVerwijderen(e) { const rowCol = document.getElementById(e.target.id).closest('.item-tafel-inhoud'); const conf = confirm('Weet u zeker dat het Item: ' + rowCol.cells[2].innerHTML + '\n Verwijderen moet worden ?'); if(conf) { return true; } else { if(sessionStorage.scrollPos)  { sessionStorage.removeItem('scrollPos'); } return false; } }
</script>